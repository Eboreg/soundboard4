package us.huseli.soundboard4

object Constants {
    const val CATEGORY_NAME_FONT_SIZE = 16
    const val DEFAULT_COLUMN_COUNT_PORTRAIT = 4
    const val PREF_AUTO_IMPORT = "autoImport"
    const val PREF_AUTO_IMPORT_CATEGORY_ID = "autoImportCategoryId"
    const val PREF_AUTO_IMPORT_DIRECTORY = "autoImportDirectory"
    const val PREF_COLUMN_COUNT_PORTRAIT = "columnCountPortrait"
    const val PREF_CONVERT_TO_WAV = "convertToWav"
    const val PREF_REPRESS_MODE = "repressMode"
    const val SOUND_DIRNAME = "sounds"
    const val SOUND_DURATION_FONT_SIZE = 10
    const val SOUND_DURATION_FONT_SIZE_LARGE = 12
    const val SOUND_NAME_FONT_SIZE = 15
    const val SOUND_NAME_FONT_SIZE_LARGE = 18
}
